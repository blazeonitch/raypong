...
void UpdateMenu(GameScreen *screen)
{
  ...
  if (CheckCollisionPointRec(mouse, quitBtn) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
  {
    *screen = EXIT;
  }
...
void DrawMenu(void)
{
  ...
  // Title
  DrawText("cPOONG", width / 2 - MeasureText("cPOONG", 100) / 2, 100, 100, DARKBLUE);
  ...
  DrawRectangleRec(startBtn, CheckCollisionPointRec(mouse, startBtn) ? hoverColor : normalColor);
  DrawText("Start Game", startBtn.x + 20, startBtn.y + 10, 20, BLACK);

  DrawRectangleRec(quitBtn, CheckCollisionPointRec(mouse, quitBtn) ? hoverColor : normalColor);
