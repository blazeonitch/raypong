...
int main(void)
{
    ...
    // -- Player Setup --
    struct GameObject player1 = {.name = "Blaise", .speed = 400, .position = {100.0f, 100.0f}};
    struct GameObject player2 = {.name = "Dick_Head", .speed = 600, .position = {screenWidth - 100.0f, 100.0f}};
    ...
    while (!WindowShouldClose() && currentScreen != EXIT)
    {
        ...
        // Smoothly interpolate timeScale toward slowMoTarget for smooth slow-mo effects
        ...
        // Scaled delta time used for all game movement and logic to apply slow-mo effect
        ...
        if (!isGamePaused)
        {
            ...
            // Clamp paddle movement to screen bounds, scaled by texture height and scale
            ...
            if ((ballPosition.y - ballRadius <= 0 && ballVelocity.y < 0) || (ballPosition.y + ballRadius >= screenHeight && ballVelocity.y > 0))
            {
                ballVelocity.y *= -1;
            }

            // Ball collision with left and right walls - invert x velocity
            if ((ballPosition.x - ballRadius <= 0 && ballVelocity.x < 0) || (ballPosition.x + ballRadius >= screenWidth && ballVelocity.x > 0))
            {
                ballVelocity.x *= -1;
            }
